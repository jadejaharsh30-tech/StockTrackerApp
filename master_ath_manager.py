...
def fetch_full_history_single(symbol):
    """
    Hybrid Strategy:
    1. Fetches MONTHLY data first to find the 'High' month (Fast).
    2. Then fetches DAILY data for just that specific month to get the exact date (Precise).
    """
    max_retries = 3
    ...
    for attempt in range(max_retries):
        try:
            # --- STEP 1: Fast Monthly Scan ---
            # Downloads only ~250 rows instead of ~5000
            df_monthly = yf.download(
                symbol, 
                period="max", 
                interval="1mo", 
                auto_adjust=False, 
            ...
            if df_monthly.empty: raise ValueError("Empty Data")

            # Handle MultiIndex if present
            if isinstance(df_monthly.columns, pd.MultiIndex):
                highs_monthly = df_monthly['High'][symbol]
            else:
                highs_monthly = df_monthly['High']

            # Find the Max Price and the "Month Start Date"
            ath_price = highs_monthly.max()
            ath_month_start = highs_monthly.idxmax() # e.g., 2023-01-01

            # --- STEP 2: Drill Down for Exact Date ---
            # Define the specific month window (Start of month to Start of next month)
            start_date = ath_month_start
            end_date = ath_month_start + pd.DateOffset(days=32) 

            # Download daily data for JUST this month
            ...
            if df_daily.empty:
                # If daily fails, fallback to month start (Better than crashing)
                return ath_price, ath_month_start

            if isinstance(df_daily.columns, pd.MultiIndex):
                highs_daily = df_daily['High'][symbol]
            else:
                highs_daily = df_daily['High']

            # Get the exact date of the high
            exact_ath_date = highs_daily.idxmax()
            
            return ath_price, exact_ath_date
