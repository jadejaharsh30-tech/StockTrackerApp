...
- **`LOOKBACK` is a ROW COUNT, not a bar offset.** `iloc[-LOOKBACK:]` holds that many rows, so the anchor lands `LOOKBACK - 1` bars before the latest bar — `RS_BARS_BACK` is derived from it for exactly that reason. **There is an open two-bar gap with the indicator**: `LOOKBACK = 211` (210 bars back) against the Pine `barsBackInput` default of 212. It stays at 211 because that is the value that has historically agreed with the chart, and the suspected cause of the recent mismatch is bad post-market yfinance data rather than the window. **Only a live-market comparison can settle it** — do not change the value off a post-market run. `test_rs_matches_pine.py` asserts the anchor lands where `RS_BARS_BACK` says and prints the gap; run it after touching anything in the RS path, and keep `RS_FULL_WINDOW` in `templates/ath_scanner.html` equal to `LOOKBACK`.
