...
- **RS history must be fetched with `auto_adjust=True`** so the stock series is split-adjusted like the index series already is. Mixing an unadjusted stock with an adjusted index puts a cliff in the ratio at every split, and since the anchor sits at the window start, everything after the split reads as a collapse — which surfaces as a false `N` on ATH O.P., the hard gate for FUND/PROP. ATH *price* detection deliberately stays unadjusted (`auto_adjust=False`), because `previous_ath` baselines are maintained in raw prices.
- RS uses the last `LOOKBACK` (211) aligned sessions, or the whole series when a recent listing has fewer, down to `MIN_RS_SESSIONS` (20). The window actually used is stored as `rs_window` and marked with `*` in the UI.
