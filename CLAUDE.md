...
- **`LOOKBACK` is a ROW COUNT, not a bar offset.** `iloc[-LOOKBACK:]` holds that many rows, so the anchor lands `LOOKBACK - 1` bars before the latest bar — `RS_BARS_BACK` is derived from it for exactly that reason. **There is an open two-bar gap with the indicator**: `LOOKBACK = 211` (210 bars back) against the Pine `barsBackInput` default of 212. It stays at 211 because that is the value that has historically agreed with the chart, and the suspected cause of the recent mismatch is bad post-market yfinance data rather than the window. **Neither value is provably right on paper.** `LOOKBACK` counts rows of the stock/index INNER JOIN, so every session missing from `^CRSLDX` pushes the anchor one stock-bar further back than the row count implies — 211 rows lands on the indicator's 212-bar anchor exactly when the index is missing two sessions in the window. Settle it with `rs_anchor_date`, which every result row carries and the RS tooltip shows: hover that date on the chart and count bars back to it. **Do this during live market** — do not change the value off a post-market run. `test_rs_matches_pine.py` asserts the anchor lands where `RS_BARS_BACK` says and prints the gap; run it after touching anything in the RS path, and keep `RS_FULL_WINDOW` in `templates/ath_scanner.html` equal to `LOOKBACK`.
- A recent listing with fewer aligned sessions uses the whole series instead, down to `MIN_RS_SESSIONS` (20). The window used is stored as `rs_window` and marked `*` in the UI. **This matches the indicator rather than departing from it**: Pine's anchor is `if bar_index == last_bar_index - barsBackInput ... else if bar_index == 0`, and on a listing shorter than `barsBackInput` the first condition never matches, so it anchors at the first bar — the same thing slicing the whole series does. Starred rows ARE comparable to the chart.
- Three deliberate departures from the Pine source, all affecting verdicts rather than RS values: green candle is `close >= prev_close` here but `close > close[1]` in Pine (differs only on an exactly unchanged close), the RS verdict allows 0.01% slack (`>= max * 0.9999`) where Pine compares exactly, and Pine forward-fills the comparison symbol onto the stock's bars whereas `calculate_rs_outperformance` inner-joins and drops sessions the two don't share.
